...
$env:PLAYER_ARGUMENTS='--cache=yes --force-seekable=yes --title="${media-title}-${metadata/date}" --ytdl-raw-options=embed-metadata=,no-check-certificate=,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang="(en|zh).*"'
...
Function mpv-fast-4k {
  Invoke-Expression "mpv.com --ytdl-format=`"bestvideo[height<=2160][protocol^=m3u]+bestaudio[protocol^=m3u]/bestvideo[height<=2160]+bestaudio/best[height<=2160][protocol^=m3u8]/best[height<=2160]`" $env:PLAYER_ARGUMENTS $args"
}
Function mpv-fast-2k {
  Invoke-Expression "mpv.com --ytdl-format=`"bestvideo[height<=1440][protocol^=m3u]+bestaudio[protocol^=m3u]/bestvideo[height<=1440]+bestaudio/best[height<=2160][protocol^=m3u8]/best[height<=2160]`" $env:PLAYER_ARGUMENTS $args"
}
Function mpv-fast-1080 {
  Invoke-Expression "mpv.com --ytdl-format=`"bestvideo[height<=1080][protocol^=m3u]+bestaudio[protocol^=m3u]/bestvideo[height<=1080]+bestaudio/best[height<=2160][protocol^=m3u8]/best[height<=2160]`" $env:PLAYER_ARGUMENTS $args"
}
Function mpv-1080 {
