...
$env:PLAYER_ARGUMENTS='--cache=yes --cache-on-disk=yes --cache-dir=%TEMP%/mpv --force-seekable=yes --title="${media-title}-${metadata/date}" --ytdl-raw-options=embed-metadata=,no-check-certificate=,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang="(en|zh).*"'
$env:STREAM_PLAYER_ARGUMENTS='--cache=yes --cache-on-disk=yes --cache-dir=%TEMP%/mpv --force-seekable=yes'
Function aria2c {
