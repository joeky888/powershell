...
$env:PLAYER_ARGUMENTS = '--cache=yes --cache-on-disk=yes --demuxer-cache-dir="' + $env:TEMP + '/mpv" --force-seekable=yes --title="${media-title}-${metadata/date}" --ytdl-raw-options=embed-metadata=,no-check-certificate=,yes-playlist=,hls-prefer-native=,ignore-errors=,write-auto-sub=,write-sub=,sub-lang="(en|zh).*"'
$env:STREAM_PLAYER_ARGUMENTS = "--cache=yes --cache-on-disk=yes --demuxer-cache-dir=$env:TEMP/mpv --force-seekable=yes"
Function aria2c {
...
Function yt-dlp-4k {
  Invoke-Expression "yt-dlp.exe $env:DLARGUMENTS -f 'bv[height<=2160]+ba/best' $args"
}

Function yt-dlp-2k {
  Invoke-Expression "yt-dlp.exe $env:DLARGUMENTS -f 'bv[height<=1440]+ba/best' $args"
}

Function yt-dlp-1080 {
  Invoke-Expression "yt-dlp.exe $env:DLARGUMENTS -f 'bv[height<=1080]+ba/best' $args"
}
Function yt-dlp-720 {
  Invoke-Expression "yt-dlp.exe $env:DLARGUMENTS -f 'bv[height<=720][fps<=30][vcodec!^=av01]+ba/best' $args"
}
...
Function mpv-720 {
  Invoke-Expression "mpv.com --ytdl-format=`"bv[height<=720][fps<=30][vcodec!^=av01]+ba/best`" $env:PLAYER_ARGUMENTS $args"
}
Function mpv-480 {
  Invoke-Expression "mpv.com --ytdl-format=`"bv[height<=480][fps<=30][vcodec!^=av01]+ba/best`" $env:PLAYER_ARGUMENTS $args"
}
Function mpv-4by3 {
