...
Function mpv-1080 {
  mpv.com --ytdl-format="bestvideo[height<=1080][fps<=30]+bestaudio/best" --cache=yes --cache-secs=36000 --cache-dir=$env:TEMP --cache-on-disk=yes $args
}
Function mpv-720 {
  mpv.com --ytdl-format="bestvideo[height<=720][fps<=30]+bestaudio/best" --cache=yes --cache-secs=36000 --cache-dir=$env:TEMP --cache-on-disk=yes $args
}
