...
if (Get-Command Set-PSReadlineKeyHandler -errorAction SilentlyContinue)
{
  ...
  Set-PSReadlineKeyHandler -Chord Ctrl+V,Ctrl+v,Ctrl+Shift+V,Ctrl+Shift+v -ScriptBlock {
...
$env:DOWNLOADARGS="--continue=true --timeout=12 --connect-timeout=12 --file-allocation=none --content-disposition-default-utf8=true --check-certificate=false --max-tries=10 --max-concurrent-downloads=150 --max-connection-per-server=16 --split=16 --min-split-size=1M --http-accept-gzip=true --parameterized-uri=false"
$env:DLARGUMENTS="-o '%(title)s.%(ext)s' --write-sub --all-subs --embed-subs --embed-metadata --sponsorblock-remove all --concurrent-fragments 8 --hls-prefer-native --ignore-errors"
$env:TORRENTARGS="--enable-dht=true --enable-dht6=true --bt-enable-lpd=true --bt-max-peers=0 --bt-request-peer-speed-limit=100M --seed-ratio=0 --bt-detach-seed-only=true --seed-time=0 --enable-peer-exchange=true --bt-tracker=$($(curl -s https://raw.githubusercontent.com/XIU2/TrackersListCollection/master/all.txt) -notmatch '^\s*$' -join ',') --max-upload-limit=25K"
...
Function mpv-fast-4k {
  Invoke-Expression "mpv.com --cache=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=`"$(yt-dlp.exe --get-url -f 'bestaudio[protocol^=m3u]' $args)`" `"$(yt-dlp.exe --write-sub --write-auto-sub --sub-format vtt --print '%(requested_subtitles.:.url)j' --sub-langs 'en.*,zh.*' $args | jq -r '[.[] | `"--sub-file=`" + .] | join(`" `")')`" `"$(yt-dlp.exe --get-url -f 'bestvideo[height<=2160][protocol^=m3u][vcodec!^=av01]' $args)`" --force-media-title='$(yt-dlp.exe --get-title $args)'"
}
Function mpv-fast-2k {
  Invoke-Expression "mpv.com --cache=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=`"$(yt-dlp.exe --get-url -f 'bestaudio[protocol^=m3u]' $args)`" `"$(yt-dlp.exe --write-sub --write-auto-sub --sub-format vtt --print '%(requested_subtitles.:.url)j' --sub-langs 'en.*,zh.*' $args | jq -r '[.[] | `"--sub-file=`" + .] | join(`" `")')`" `"$(yt-dlp.exe --get-url -f 'bestvideo[height<=1440][protocol^=m3u][vcodec!^=av01]' $args)`" --force-media-title='$(yt-dlp.exe --get-title $args)'"
}
Function mpv-fast-1080 {
  Invoke-Expression "mpv.com --cache=yes --ytdl-raw-options=no-check-certificate=,concurrent-fragments=8 --audio-file=`"$(yt-dlp.exe --get-url -f 'bestaudio[protocol^=m3u]' $args)`" `"$(yt-dlp.exe --write-sub --write-auto-sub --sub-format vtt --print '%(requested_subtitles.:.url)j' --sub-langs 'en.*,zh.*' $args | jq -r '[.[] | `"--sub-file=`" + .] | join(`" `")')`" `"$(yt-dlp.exe --get-url -f 'bestvideo[height<=1080][protocol^=m3u][vcodec!^=av01]' $args)`" --force-media-title='$(yt-dlp.exe --get-title $args)'"
}
Function mpv-1080 {
...
Function streamlink-download {
  streamlink.exe --loglevel debug --stream-segment-threads 10 --twitch-low-latency --http-no-ssl-verify --title '{title}' --default-stream best -o '{title}.mp4' "$args"
}
