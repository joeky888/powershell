...
Function find {
  Get-ChildItem -Recurse -Filter "*$args*" | Where-Object { $_.fullname -NotLike "*.git*" }
}
Function grep {
  ...
  if ($pipeline.length -gt 0) {
    ...
    Get-ChildItem -Recurse | Where-Object { $_.fullname -NotLike "*.git*" } | Select-String $args | Select Path, LineNumber, Line | Format-List
  }
}
Function ls {
